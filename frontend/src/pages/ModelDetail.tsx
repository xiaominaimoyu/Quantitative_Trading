...
import ModelVersionCreateModal from '@/components/research/ModelVersionCreateModal'
...
export default function ModelDetailPage() {
  ...
  const [source, setSource] = useState<ModelVersion | null>(null)
  ...
  const versionsQ = useQuery({ queryKey: ['modelVersions', modelId], queryFn: () => listModelVersions(modelId), enabled: Boolean(modelId) })

  if (modelQ.isLoading || versionsQ.isLoading) return <PageLoading />
  const error = modelQ.error ?? versionsQ.error
  if (isForbiddenError(error)) return <PageForbidden description="当前身份没有模型读取权限" />
  if (error) return <PageError error={error} retry={() => { void modelQ.refetch(); void versionsQ.refetch() }} />
  const model = modelQ.data
  if (!model) return <PageEmpty title="模型不存在" />
  const versions = versionsQ.data?.items ?? []
  const canCreateVersion = canManageOwned('model:version:create', model.ownerKey)
  ...
  return (
    <div>
      <PageHeader
        parent="策略实验室"
        title={model.name}
        subtitle={model.description ?? '未填写描述'}
        meta={[<CopyableId key="id" id={model.id} maxLength={0} />, `Owner ${model.ownerKey}`, `slug ${model.slug}`]}
      />
      {lastAuditId ? (
        <Alert style={{ marginTop: 16 }} type="success" showIcon message="版本创建完成" description={<CopyableId id={lastAuditId} maxLength={0} />} />
      ) : null}
      {!canCreateVersion ? (
        <Alert style={{ marginTop: 16 }} type="info" showIcon message="当前身份只能读取此模型" description="创建子版本需要 model:version:create 且必须是 owner；管理员可跨 owner 操作。" />
      ) : null}
      <Descriptions bordered size="small" column={2} style={{ marginTop: 16 }}>
        <Descriptions.Item label="版本数">{model.versionCount}</Descriptions.Item>
        <Descriptions.Item label="最新状态">{model.latestVersionStatus ? RESEARCH_VERSION_STATUS_LABEL[model.latestVersionStatus] : '暂无版本'}</Descriptions.Item>
        <Descriptions.Item label="创建时间">{formatDateTime(model.createdAt, { zone: false })}</Descriptions.Item>
        <Descriptions.Item label="更新时间">{formatDateTime(model.updatedAt, { zone: false })}</Descriptions.Item>
      </Descriptions>
      ...
      {versions.length === 0 ? (
        <PageEmpty
          title="暂无模型版本"
          description="创建首个严格无预测基线草稿；冻结后才能作为后续版本父节点。"
        ...
        <div className="qt-table-scroll">
          <Table
            ...
            pagination={false}
            columns={[
              { title: '版本', dataIndex: 'version', width: 80, render: (value, row) => <Link to={`/models/${modelId}/versions/${row.id}`}>v{value}</Link> },
              ...
              { title: '创建时间', dataIndex: 'createdAt', width: 170, render: (value) => formatDateTime(value, { zone: false }) },
              {
                ...
                render: (_, row) => (
                  <Space size={0}>
                    <Button type="link" size="small" onClick={() => navigate(`/models/${modelId}/versions/${row.id}`)}>查看</Button>
      ...
      <ModelVersionCreateModal
        ...
        onCreated={async (result) => {
          ...
          await modelQ.refetch()
          message.success('模型草稿版本已创建')
          navigate(`/models/${modelId}/versions/${result.item.id}`, { state: { auditEventId: result.auditEventId } })
        }}
      />
    </div>
  )
